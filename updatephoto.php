		...
		if (!isset($_FILES['image']['tmp_name'])) {
			...
			if ($error > 0) {
				...
				if ($size > 10000000) //conditions for the file
				{
					...
					$newImgName = $_FILES["image"]["name"];
					move_uploaded_file($_FILES["image"]["tmp_name"], "upload/" . $newImgName);
					$location = "upload/" . $newImgName;
					$user = $_SESSION['id'];
					...
					$query = "UPDATE user SET profile_picture = '$location' WHERE email='$email'";
					$result = mysqli_query($conn, $query);


					$query2 = "UPDATE `users` SET `img` = '$location' WHERE `email` = '$email'";
					$result = mysqli_query($conn, $query2);
					echo mysqli_error($conn);

					$query1 = "UPDATE comments SET image = '$location' WHERE user_id='$user'";
